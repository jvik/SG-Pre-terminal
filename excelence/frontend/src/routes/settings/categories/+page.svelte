...
<div class="min-h-screen bg-gray-50 dark:bg-slate-900">
	<div class="max-w-6xl mx-auto p-8">
		<div class="mb-8">
			...
			<p class="text-gray-600 dark:text-slate-400">Organize your transactions with custom categories</p>
		</div>
		...
		{#if isLoading}
			<div class="bg-white dark:bg-slate-800 shadow-lg rounded-lg p-12 text-center">
		...
		{:else}
		<div class="bg-white dark:bg-slate-800 shadow-lg rounded-lg overflow-hidden border border-gray-200 dark:border-slate-700">
			{#if categories.length === 0}
				<div class="p-8 text-center text-gray-500 dark:text-slate-400">
					<p class="text-lg">No categories yet</p>
					<p class="text-sm mt-2">Click "Add Category" to create your first category</p>
				</div>
			{:else}
				<table class="min-w-full">
					<thead>
						<tr class="bg-gray-50 dark:bg-slate-700 border-b border-gray-200 dark:border-slate-600">
							<th class="px-6 py-4 text-left text-xs font-semibold text-gray-600 dark:text-slate-300 uppercase tracking-wider w-20">Icon</th>
							<th class="px-6 py-4 text-left text-xs font-semibold text-gray-600 dark:text-slate-300 uppercase tracking-wider">Category Name</th>
							<th class="px-6 py-4 text-right text-xs font-semibold text-gray-600 dark:text-slate-300 uppercase tracking-wider w-48">Actions</th>
						</tr>
					</thead>
...
{#if showModal}
	...
	>
		<div class="bg-white dark:bg-slate-800 rounded-xl shadow-2xl w-full max-w-md" on:click|stopPropagation on:keydown|stopPropagation role="dialog" tabindex="-1">
			...
			<div class="px-6 py-5 space-y-5">
				{#if modalError}
					<div class="bg-red-50 dark:bg-red-900/20 border-l-4 border-red-500 dark:border-red-400 p-4 rounded">
						<p class="text-red-700 dark:text-red-400 text-sm">{modalError}</p>
					</div>
				{/if}

				<div>
					<label for="category-emoji" class="block text-sm font-semibold text-gray-700 dark:text-slate-300 mb-3"
						>Icon</label
					>
					<div class="flex items-center gap-3 mb-3">
						<input
							...
							placeholder="📂"
						/>
						<span class="text-sm text-gray-500 dark:text-slate-400">or choose from popular icons:</span>
					</div>
					<div class="grid grid-cols-9 gap-2">
						{#each commonEmojis as emoji}
							<button
								...
								class="text-2xl p-2 rounded-lg transition-all duration-150 {categoryEmojiInput === emoji
				...
				<div>
					<label for="category-name" class="block text-sm font-semibold text-gray-700 dark:text-slate-300 mb-2"
						>Category Name</label
					>
					<input
						id="category-name"
		...
		<div class="bg-white dark:bg-slate-800 rounded-xl shadow-2xl w-full max-w-md" on:click|stopPropagation on:keydown|stopPropagation role="dialog" tabindex="-1">
			<div class="px-6 py-5 border-b border-gray-200 dark:border-slate-700">
				<div class="flex items-center">
					...
					<h2 class="ml-3 text-2xl font-bold text-gray-900 dark:text-white">
						Confirm Deletion
					</h2>
				</div>
			</div>
			...
			<div class="px-6 py-5">
				...
				<div class="mt-3 bg-yellow-50 dark:bg-yellow-900/20 border-l-4 border-yellow-400 dark:border-yellow-500 p-4 rounded">
					<p class="text-sm text-yellow-700 dark:text-yellow-400">
						<strong>Note:</strong> Deletion will be blocked if the category is in use by any transactions.
					</p>
				</div>
			</div>
